- [![](data:image/svg+xml;base64,){.breadcrumbHomeIcon_YNFT}](https://tradingview.github.io/lightweight-charts/){.breadcrumbs__link
  aria-label="Home page"}
- [Time scale]{.breadcrumbs__link itemprop="name"}

[Version: 5.1]{.theme-doc-version-badge .badge .badge--secondary}

On this page

# Time scale

## Overview[​](time-scale.html#overview "Direct link to Overview"){.hash-link aria-label="Direct link to Overview"} {#overview .anchor .anchorWithStickyNavbar_LWe7}

Time scale (or time axis) is a horizontal scale that displays the time
of data points at the bottom of the chart.

![Time
scale](data:image/png;base64, "Time scale"){.img_ev3q
decoding="async" loading="lazy" width="687" height="235"}

The horizontal scale can also represent price or other custom values.
Refer to the [Chart types](chart-types.html) article for more
information.

### Time scale appearance[​](time-scale.html#time-scale-appearance "Direct link to Time scale appearance"){.hash-link aria-label="Direct link to Time scale appearance"} {#time-scale-appearance .anchor .anchorWithStickyNavbar_LWe7}

Use [`TimeScaleOptions`](api/interfaces/TimeScaleOptions.html) to adjust
the time scale appearance. You can specify these options in two ways:

- On chart initialization. To do this, provide the desired options as a
  [`timeScale`](api/interfaces/ChartOptionsBase.html#timescale)
  parameter when calling
  [`createChart`](api/functions/createChart.html).
- On the fly using either the
  [`ITimeScaleApi.applyOptions`](api/interfaces/ITimeScaleApi.html#applyoptions)
  or
  [`IChartApi.applyOptions`](api/interfaces/IChartApi.html#applyoptions)
  method. Both methods produce the same result.

### Time scale API[​](time-scale.html#time-scale-api "Direct link to Time scale API"){.hash-link aria-label="Direct link to Time scale API"} {#time-scale-api .anchor .anchorWithStickyNavbar_LWe7}

Call the
[`IChartApi.timeScale`](api/interfaces/IChartApi.html#timescale) method
to get an instance of the
[`ITimeScaleApi`](api/interfaces/ITimeScaleApi.html) interface. This
interface provides an extensive API for controlling the time scale. For
example, you can adjust the visible range, convert a time point or
[index](api/type-aliases/Logical.html) to a coordinate, and subscribe to
events.

``` {.prism-code .language-javascript .codeBlock_bY9V .thin-scrollbar tabindex="0" style="color:#393A34;background-color:#f6f8fa"}
chart.timeScale().resetTimeScale();
```

[![](data:image/svg+xml;base64,){.copyButtonIcon_y97N}![](data:image/svg+xml;base64,PHN2ZyB2aWV3Ym94PSIwIDAgMjQgMjQiIGNsYXNzPSJjb3B5QnV0dG9uU3VjY2Vzc0ljb25fTGpkUyI+PHBhdGggZmlsbD0iY3VycmVudENvbG9yIiBkPSJNMjEsN0w5LDE5TDMuNSwxMy41TDQuOTEsMTIuMDlMOSwxNi4xN0wxOS41OSw1LjU5TDIxLDdaIiAvPjwvc3ZnPg==){.copyButtonSuccessIcon_LjdS}]{.copyButtonIcons_eSgA
aria-hidden="true"}

## Visible range[​](time-scale.html#visible-range "Direct link to Visible range"){.hash-link aria-label="Direct link to Visible range"} {#visible-range .anchor .anchorWithStickyNavbar_LWe7}

Visible range is a chart area that is currently visible on the canvas.
This area can be measured with both [data](time-scale.html#data-range)
and [logical](time-scale.html#logical-range) range. Data range usually
includes bar timestamps, while logical range has bar indices.

You can adjust the visible range using the following methods:

- [`setVisibleRange`](api/interfaces/ITimeScaleApi.html#setvisiblerange)
- [`getVisibleRange`](api/interfaces/ITimeScaleApi.html#getvisiblerange)
- [`setVisibleLogicalRange`](api/interfaces/ITimeScaleApi.html#setvisiblelogicalrange)
- [`getVisibleLogicalRange`](api/interfaces/ITimeScaleApi.html#getvisiblelogicalrange)

### Data range[​](time-scale.html#data-range "Direct link to Data range"){.hash-link aria-label="Direct link to Data range"} {#data-range .anchor .anchorWithStickyNavbar_LWe7}

The data range includes only values from the first to the last bar
visible on the chart. If the visible area has empty space, this part of
the scale is not included in the data range.

Note that you cannot extrapolate time with the
[`setVisibleRange`](api/interfaces/ITimeScaleApi.html#setvisiblerange)
method. For example, the chart does not have data prior `2018-01-01`
date. If you set the visible range from `2016-01-01`, it will be
automatically adjusted to `2018-01-01`.

If you want to adjust the visible range more flexible, operate with the
[logical range](time-scale.html#logical-range) instead.

### Logical range[​](time-scale.html#logical-range "Direct link to Logical range"){.hash-link aria-label="Direct link to Logical range"} {#logical-range .anchor .anchorWithStickyNavbar_LWe7}

The logical range represents a continuous line of values. These values
are logical [indices](api/type-aliases/Logical.html) on the scale that
illustrated as red lines in the image below:

![Logical
range](data:image/png;base64, "Logical range"){.img_ev3q
decoding="async" loading="lazy" width="595" height="299"}

The logical range starts from the first data point across all series,
with negative indices before it and positive ones after.

The indices can have fractional parts. The integer part represents the
fully visible bar, while the fractional part indicates partial
visibility. For example, the `5.2` index means that the fifth bar is
fully visible, while the sixth bar is 20% visible. A half-index, such as
`3.5`, represents the middle of the bar.

In the library, the logical range is represented with the
[`LogicalRange`](api/type-aliases/LogicalRange.html) object. This object
has the `from` and `to` properties, which are logical indices on the
time scale. For example, the visible logical range on the chart above is
approximately from `-4.73` to `5.05`.

The
[`setVisibleLogicalRange`](api/interfaces/ITimeScaleApi.html#setvisiblelogicalrange)
method allows you to specify the visible range beyond the bounds of the
available data. This can be useful for setting a [chart
margin](time-scale.html#chart-margin) or aligning series visually.

## Chart margin[​](time-scale.html#chart-margin "Direct link to Chart margin"){.hash-link aria-label="Direct link to Chart margin"} {#chart-margin .anchor .anchorWithStickyNavbar_LWe7}

Margin is the space between the chart\'s borders and the series. It
depends on the following time scale options:

- [`barSpacing`](api/interfaces/TimeScaleOptions.html#barspacing). The
  default value is `6`.
- [`rightOffset`](api/interfaces/TimeScaleOptions.html#rightoffset). The
  default value is `0`.

You can specify these options as described
[above](time-scale.html#time-scale-appearance).

Note that if a series contains only a few data points, the chart may
have a large margin on the left side.

![A series with a few
points](../assets/images/extra-margin-c1f99247fd0192e01717a0e48e073f13.png){.img_ev3q
decoding="async" loading="lazy" width="1358" height="408"}

In this case, you can call the
[`fitContent`](api/interfaces/ITimeScaleApi.html#fitcontent) method that
adjust the view and fits all data within the chart.

``` {.prism-code .language-javascript .codeBlock_bY9V .thin-scrollbar tabindex="0" style="color:#393A34;background-color:#f6f8fa"}
chart.timeScale().fitContent();
```

[![](data:image/svg+xml;base64,){.copyButtonIcon_y97N}![](data:image/svg+xml;base64,PHN2ZyB2aWV3Ym94PSIwIDAgMjQgMjQiIGNsYXNzPSJjb3B5QnV0dG9uU3VjY2Vzc0ljb25fTGpkUyI+PHBhdGggZmlsbD0iY3VycmVudENvbG9yIiBkPSJNMjEsN0w5LDE5TDMuNSwxMy41TDQuOTEsMTIuMDlMOSwxNi4xN0wxOS41OSw1LjU5TDIxLDdaIiAvPjwvc3ZnPg==){.copyButtonSuccessIcon_LjdS}]{.copyButtonIcons_eSgA
aria-hidden="true"}

If calling `fitContent` has no effect, it might be due to how the
library displays data.

The library allocates specific width for each data point to maintain
consistency between different chart types. For example, for line series,
the plot point is placed at the center of this allocated space, while
candlestick series use most of the width for the candle body. The
allocated space for each data point is proportional to the chart width.
As a result, series with fewer data points may have a small margin on
both sides.

![Margin](../assets/images/margin-ba3ba588b8212ea7b6415220ab39424a.png){.img_ev3q
decoding="async" loading="lazy" width="1362" height="408"}

You can specify the [logical range](time-scale.html#logical-range) with
the
[`setVisibleLogicalRange`](api/interfaces/ITimeScaleApi.html#setvisiblelogicalrange)
method to display the series exactly to the edges. For example, the code
sample below adjusts the range by half a bar-width on both sides.

``` {.prism-code .language-javascript .codeBlock_bY9V .thin-scrollbar tabindex="0" style="color:#393A34;background-color:#f6f8fa"}
const vr = chart.timeScale().getVisibleLogicalRange();
chart.timeScale().setVisibleLogicalRange({ from: vr.from + 0.5, to: vr.to - 0.5 });
```

[![](data:image/svg+xml;base64,){.copyButtonIcon_y97N}![](data:image/svg+xml;base64,PHN2ZyB2aWV3Ym94PSIwIDAgMjQgMjQiIGNsYXNzPSJjb3B5QnV0dG9uU3VjY2Vzc0ljb25fTGpkUyI+PHBhdGggZmlsbD0iY3VycmVudENvbG9yIiBkPSJNMjEsN0w5LDE5TDMuNSwxMy41TDQuOTEsMTIuMDlMOSwxNi4xN0wxOS41OSw1LjU5TDIxLDdaIiAvPjwvc3ZnPg==){.copyButtonSuccessIcon_LjdS}]{.copyButtonIcons_eSgA
aria-hidden="true"}

[](price-scale.html){.pagination-nav__link .pagination-nav__link--prev}

Previous

Price scale

[](panes.html){.pagination-nav__link .pagination-nav__link--next}

Next

Panes

- [Overview](time-scale.html#overview){.table-of-contents__link
  .toc-highlight}
  - [Time scale
    appearance](time-scale.html#time-scale-appearance){.table-of-contents__link
    .toc-highlight}
  - [Time scale
    API](time-scale.html#time-scale-api){.table-of-contents__link
    .toc-highlight}
- [Visible
  range](time-scale.html#visible-range){.table-of-contents__link
  .toc-highlight}
  - [Data range](time-scale.html#data-range){.table-of-contents__link
    .toc-highlight}
  - [Logical
    range](time-scale.html#logical-range){.table-of-contents__link
    .toc-highlight}
- [Chart margin](time-scale.html#chart-margin){.table-of-contents__link
  .toc-highlight}
