...
* La clave representa **una sola serie escalar**.
* La clave puede ser:
  + `name@version` para features versionadas,
  + `name` para series externas simples persistidas sin version.
...
* trazabilidad de degradacion.

Nota:

Las series externas McClellan se guardan deliberadamente sin `@version` para priorizar simplicidad operacional. La semantica completa sigue existiendo en `FeatureSpec`; la persistencia solo usa una clave simple.
...
* La clave debe ser un identificador simple o `name@version`.
...
Hoy `load_market_data()` reconstruye unicamente columnas de valor, una por feature key. La calidad se conserva en disco pero no se expone todavia como columnas paralelas al DataFrame de salida.